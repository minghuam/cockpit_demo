...
deploy_file = '/home/minghuam/egocentric_action_recognition/obj/obj_cropped/obj_deploy_gtea.prototxt'
model_file = '/home/minghuam/egocentric_action_recognition/obj/obj_cropped/GTEA_model/OBJ_GTEA_S1_iter_3000.caffemodel'
test_file = '/home/minghuam/egocentric_action_recognition/obj/obj_cropped/GTEA_data/S1_test.txt'
mean_file = '/home/minghuam/egocentric_action_recognition/obj/obj_cropped/image_mean.npy'
max_activations_dir = 'gtea_hand'
...
with open(test_file, 'r') as fr:
    for line in fr.readlines():
        tokens = line.strip().split(' ')
        image_pairs.append((tokens[0], int(tokens[1])))
...
def viz_layer(net, layer, Iraw, filter_index, max_activations, raw_img):
    '''C x H x W'''
    ...
    while key != ord('q'):
        ...
        if key == ord('s'):
            print 'filter', filter_index
            folder = os.path.join('obj_examples/white', os.path.splitext(os.path.basename(raw_img))[0])
            if not os.path.exists(folder):
                os.mkdir(folder)
            cv2.imwrite(os.path.join(folder, 'Ia.jpg'), Ia)
            cv2.imwrite(os.path.join(folder, 'Iraw.jpg'), Iraw)
            cv2.imwrite(os.path.join(folder, 'Iunit.jpg'), Iunit)
            cv2.imwrite(os.path.join(folder, 'I.jpg'), cv2.imread(raw_img))

    return filter_index

crop_size = 224
Imean = np.load(mean_file)
x = (Imean.shape[1] - crop_size)/2
y = (Imean.shape[0] - crop_size)/2
Imean = Imean[y:y+crop_size, x:x+crop_size, :]

filter_index = 0
for (raw_img, label) in image_pairs[3195:]:
    print raw_img, label
    ...
    filter_index = viz_layer(net, 'conv5', I, filter_index, max_activations, raw_img)